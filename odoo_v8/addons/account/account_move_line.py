...
class account_move_line(osv.osv):
    ...
    def _query_get(self, cr, uid, obj='l', context=None):
        ...
        if context.get('partner_ids'):
            query_params['partner_ids'] = tuple(context['partner_ids'])
            query += ' AND '+obj+'.partner_id IN %(partner_ids)s'
    ...
    def _prepare_analytic_line(self, cr, uid, obj_line, context=None):
        ...
        return {'name': obj_line.name,
                'date': self.pool.get('avancys.orm2sql').local_date(datetime.strftime(datetime.strptime(obj_line.date,'%Y-%m-%d'),'%Y-%m-%d %H:%M:%S')),
    ...
    def reconcile_partial(self, cr, uid, ids, type='auto', context=None, writeoff_acc_id=False, writeoff_period_id=False, writeoff_journal_id=False):
        ...
        if context is None:
            context = {}
        for line in self.browse(cr, uid, ids, context=context):
            if line.id and company_list and not line.company_id.id in company_list:
    ...
    def create(self, cr, uid, vals, context=None, check=True):
        ...
        journal_obj = self.pool.get('account.journal')
        context = dict(context or {})
        if not vals['debit'] > 0 and not vals['credit'] > 0:
            return
