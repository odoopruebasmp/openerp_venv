...
class account_invoice_line(osv.osv):
    ...
    def product_id_change(self, cr, uid, ids, product, uom_id, qty=0, name='', type='out_invoice', partner_id=False,
                          ...
                          account_analytic_id=False, context=None):
        res_prod = super(account_invoice_line, self).product_id_change(cr, uid, ids, product, uom_id, qty, name,
                                                    type, partner_id, fposition_id, price_unit, currency_id=currency_id,
                                                    company_id=company_id, account_analytic_id=account_analytic_id,
                                                    context=context)
        if self.pool.get('res.company').browse(cr, uid, company_id).analytic_default:
