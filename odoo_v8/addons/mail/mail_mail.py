...
class mail_mail(osv.Model):
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        #The next line is commented so that the route of the input msn is not deleted
        # self.pool.get('mail.message').unlink(cr, uid, parent_msg_ids, context=context)
        for p_m_id in parent_msg_ids:
            msg_id = self.pool.get('mail.message').browse(cr, uid, p_m_id, context=context)
            if msg_id.type == 'notification':
                self.pool.get('mail.message').unlink(cr, uid, p_m_id, context=context)
        return res
