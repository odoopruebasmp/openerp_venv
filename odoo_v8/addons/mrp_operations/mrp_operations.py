...
class mrp_production_workcenter_line(osv.osv):
    ...
    _columns = {
       ...
       'paused_time': fields.float('Tiempo pausado'),
       'last_paused': fields.datetime('Ultima pausa'),
    ...
    def action_pause(self, cr, uid, ids, context=None):
        ...
        """
        obj_line = self.browse(cr, uid, ids[0])
        obj_line.last_paused = time.strftime('%Y-%m-%d %H:%M:%S')
    ...
    def action_resume(self, cr, uid, ids, context=None):
        ...
        """
        obj_line = self.browse(cr, uid, ids[0])
        paused = obj_line.paused_time
        if obj_line.state == 'pause' and obj_line.last_paused:
            date_now = time.strftime('%Y-%m-%d %H:%M:%S')
            date_start = datetime.strptime(obj_line.last_paused, '%Y-%m-%d %H:%M:%S')
            date_finished = datetime.strptime(date_now, '%Y-%m-%d %H:%M:%S')
            dif = date_finished - date_start
            paused += dif.days * 24
            paused += dif.seconds / float(3600)

        return self.write(cr, uid, ids, {
            'state': 'startworking',
            'paused_time': paused,
            'last_paused': False}, context=context)
