    ...
    def onchange(self, values, field_name, field_onchange):
        """ Perform an onchange on the given field.
            ...
            :param field_onchange: dictionary mapping field names to their
                on_change attribute
        """
        self = self.with_context(field_onchange=field_name)
        field_2many = ''
        env = self.env
        ...
        while todo:
            ...
            with env.do_in_onchange():
                ...
                if field_onchange.get(name):
                    field_2many = record._onchange_eval(name, field_onchange[name], result)
                            ...
                            if field.type == 'many2many':
                                for n in newval:
                                    if name not in result['value']:
                                        result['value'][name] = [n.id]
                                        todo.append(name)
                                    else:
                                        if n.id not in result['value'][name]:
                                            result['value'][name].append(n.id)
                                            todo.append(name)
                            else:
                                ...
                                result['value'][name] = field.convert_to_write(
                                        newval, record._origin, subfields.get(name), bypass)
                                todo.append(name)
                        else:
        ...
        if field_name and not isinstance(field_name, list) and \
                self._fields[field_name].type in ('one2many', 'many2many') and field_name != field_2many:
