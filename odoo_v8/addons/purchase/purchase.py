...
class purchase_order(osv.osv):
    ...
    def _prepare_inv_line(self, cr, uid, account_id, order_line, context=None):
        ...
        return {
            ...
            'discount': order_line.discount or 0,
    ...
    def _create_stock_moves(self, cr, uid, order, order_lines, picking_id=False, context=None):
        ...
        for order_line in order_lines:
            ...
            if order_line.product_id.type in ('product', 'consu', 'service'):
        ...
        * Order lines are exactly the same except for the quantity and unit
        ...
        for porder in [order for order in self.browse(cr, uid, ids, context=context) if order.state == 'draft']:
            ...
            if not order_infos:
                ...
                if getattr(porder, 'requisition_idss', None):
                    order_infos['requisition_idss'] = [(6, 0, list(porder.requisition_idss._ids))]
            else:
                ...
                if getattr(porder, 'requisition_idss', None):
                    order_infos['requisition_idss'] = [(6, 0, order_infos['requisition_idss'][0][2] + \
                                                    list(porder.requisition_idss._ids))]
            ...
            if po.invoice_method == 'picking':
                ...
                for po_line in po.order_line:
                    if (po_line.move_ids and
                        ...
                        is_invoiced.append(po_line.id)
                    elif po_line.price_unit < 0.0001:
                        is_invoiced.append(po_line.id)
            else:
                for po_line in po.order_line:
    ...
    def onchange_product_id(self, cr, uid, ids, pricelist_id, product_id, qty, uom_id,
        ...
        if price_unit is False or price_unit is None:
            ...
            if pricelist_id:
                ...
                context_partner.update( {'uom': uom_id, 'date': date_order_str} )
                price = product_pricelist.price_get(cr, uid, [pricelist_id],
                        product.id, qty or 1.0, partner_id or False, context=context_partner)[pricelist_id]
            else:
    ...
    def make_po(self, cr, uid, ids, context=None):
        ...
        for procurement in self.browse(cr, uid, ids, context=context):
            ...
            if not partner:
                ...
                if available_draft_po_ids and procurement.rule_id.agrupar:
