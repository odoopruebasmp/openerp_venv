...
class account_journal(osv.osv):
    ...
    _columns = {
        ...
        'type': fields.selection([('sale', 'Sale'), ('sale_refund', 'Sale Refund'), ('sale_add', 'Adición en Venta'),
                                  ('purchase', 'Purchase'), ('purchase_refund', 'Purchase Refund'), ('cash', 'Cash'),
                                  ('bank', 'Bank and Checks'), ('general', 'General'),
...
class account_period(osv.osv):
    ...
    def action_draft(self, cr, uid, ids, context=None):
        ...
        date = datetime.now()
        cr.execute("update account_journal_period set write_uid=%s, write_date=%s, state=%s "
                   "where period_id in %s", (uid, str(date), mode, tuple(ids),))
        cr.execute("update account_period set write_uid=%s, write_date=%s, state=%s "
                   "where id in %s", (uid, str(date), mode, tuple(ids),))
...
class account_move(osv.osv):
    ...
    def post(self, cr, uid, ids, context=None):
        ...
        move_id = self.browse(cr, uid, ids[0])
        diff = sum([x.debit - x.credit for x in move_id.line_id])
        if -1*10**prec < round(diff, prec) < 0:
            if not move_id.company_id.expense_currency_exchange_account_id:
                raise Warning("Por favor defina la cuenta de gasto de ajuste al peso en la compañía")
            adjust_line = {
                'name': 'Movimiento de ajuste al peso / dif en cambio',
                'partner_id': move_id.company_id.id,
                'account_id': move_id.company_id.expense_currency_exchange_account_id.id,
                'move_id': move_id.id,
            ...
            self.pool.get('account.move.line').create(cr, uid, adjust_line)
        elif 0 < round(diff, prec) < 10**prec:
            if not move_id.company_id.income_currency_exchange_account_id:
                raise Warning("Por favor defina la cuenta de ingreso de ajuste al peso en la compañía")
            adjust_line = {
                'name': 'Movimiento de ajuste al peso / dif en cambio',
                'partner_id': move_id.company_id.id,
                'account_id': move_id.company_id.income_currency_exchange_account_id.id,
                'move_id': move_id.id,
                'journal_id': move_id.journal_id.id,
                'period_id': move_id.period_id.id,
                'date': move_id.date,
    ...
    def button_cancel(self, cr, uid, ids, context=None):
        for line in self.browse(cr, uid, ids, context=context):
            ...
            cr.execute('SELECT state FROM account_journal_period WHERE journal_id = %s AND period_id = %s', (journal.id, period.id))
            result = cr.fetchall()
            for (state,) in result:
                if state == 'done':
    ...
    def validate(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, ids, context):
            ...
            for line in move.line_id:
                if not obj_move_line._check_date(cr, uid, line.id, context=context):
                    raise osv.except_osv(_('Error!'), _("The date of your Journal Entry is not in the defined period! "
                                                        "You should change the date or remove this constraint from the "
                                                        "journal: %s" % line.journal_id.name))
                amount += line.debit - line.credit
