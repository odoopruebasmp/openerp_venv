...
#    _rec_name = 'number'
    ...
    def first_move_line_get(self, cr, uid, voucher_id, move_id, company_currency, current_currency, context=None):
        ...
        if voucher.type in ('purchase', 'payment'):
            ...
            account = voucher.journal_id.default_credit_account_id.id
        elif voucher.type in ('sale', 'receipt'):
            debit = voucher.paid_amount_in_company_currency
            account = voucher.journal_id.default_debit_account_id.id
        if not account and voucher.type != 'crossing':
            raise Warning("No se ha podido establecer la cuenta del movimiento contable")
    ...
    def voucher_move_line_create(self, cr, uid, voucher_id, line_total, move_id, company_currency, current_currency, context=None):
        ...
        for line in voucher.line_ids:
            ...
            if line.amount == line.amount_unreconciled:
                if line.amount_unreconciled != round(line.move_line_id.amount_residual, dp.get_precision('Account').func_globals['decimal_precision']._defaults['digits']):
                    raise osv.except_osv(_('Valores incongruentes'), ('El saldo inicial de la línea con asignación %s '
                                        'que está tratando de conciliar, no coincide con el valor residual del '
                                        'elemento del libro, debido a que ya tiene conciliaciones. Por favor presione '
                                        'el botón RECALCULAR LINEAS') % line.amount)
