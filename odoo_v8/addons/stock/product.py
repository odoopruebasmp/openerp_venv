...
class product_product(osv.osv):
    ...
    def _get_domain_locations(self, cr, uid, ids, context=None):
        ...
        locations = location_obj.browse(cr, SUPERUSER_ID, location_ids, context=context)
    ...
    def _product_available(self, cr, uid, ids, field_names=None, arg=False, context=None):
        ...
        domain_move_in += self._get_domain_dates(cr, SUPERUSER_ID, ids, context=context) + [('state', 'not in', ('done', 'cancel', 'draft'))] + domain_products
        domain_move_out += self._get_domain_dates(cr, SUPERUSER_ID, ids, context=context) + [('state', 'not in', ('done', 'cancel', 'draft'))] + domain_products
        ...
        moves_in = self.pool.get('stock.move').read_group(cr, SUPERUSER_ID, domain_move_in, ['product_id', 'product_qty'], ['product_id'], context=context)
        moves_out = self.pool.get('stock.move').read_group(cr, SUPERUSER_ID, domain_move_out, ['product_id', 'product_qty'], ['product_id'], context=context)

        domain_quant += domain_quant_loc
        quants = self.pool.get('stock.quant').read_group(cr, SUPERUSER_ID, domain_quant, ['product_id', 'qty'], ['product_id'], context=context)
