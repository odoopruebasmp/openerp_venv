    ...
    def _get_current_rate(self, cr, uid, ids, raise_on_no_rate=True, context=None):
        ...
        for cid in ids:
            cr.execute('SELECT rate FROM res_currency_rate '
                       ...
                       (cid, date))
            if cr.rowcount:
                res[cid] = cr.fetchone()[0]
            elif not raise_on_no_rate:
                res[cid] = 0
            else:
                currency = self.browse(cr, uid, cid, context=context)
                raise osv.except_osv(_('Error!'), _("No currency rate associated for currency '%s' for the given period"
                                                    % currency.name))
        return res
