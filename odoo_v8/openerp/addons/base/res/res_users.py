...
class res_groups(osv.osv):
    ...
    _columns = {
        'name': fields.char('Name', required=True, translate=True, copy=False),
    ...
    def write(self, cr, uid, ids, vals, context=None):
        if 'name' in vals:
            group_id = self.browse(cr, uid, ids)
            duplicated = self.search(cr, uid,
                                     [('name', '=', vals['name']), ('category_id', '=', group_id.category_id.id)],
                                     context=context)
            if duplicated:
                vals['name'] = str(vals['name']) + " copy"
...
class users_view(osv.osv):
    ...
    def read(self, cr, uid, ids, fields=None, context=None, load='_classic_read'):
        ...
        res = super(users_view, self).read(cr, 1, ids, other_fields, context=context, load=load)
