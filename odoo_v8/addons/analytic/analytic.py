...
class account_analytic_account(osv.osv):
    ...
    def _debit_credit_bal_qtty(self, cr, uid, ids, fields, arg, context=None):
        ...
        if context.get('date_from',False):
            ctx.update({'from_date':context.get('date_from')})
        if context.get('date_to',False):
            ctx.update({'to_date':context.get('date_to')})
        context = ctx
        ...
        if context.get('partner_ids'):
            where_clause_args +=  [tuple(context['partner_ids'])]
            where_date += ' AND l.partner_id IN %s'
        if context.get('general_account_ids'):
            where_clause_args += [tuple(context['general_account_ids'])]
            where_date += ' AND l.general_account_id IN %s'
        if context.get('period_ids'):
            where_clause_args += [tuple(context['period_ids'])]
            where_date += ' AND l.period_id IN %s'
