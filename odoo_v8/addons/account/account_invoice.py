...
class account_invoice(models.Model):
    ...
    def _default_journal(self):
        ...
        tp = ('type', 'in', filter(None, map(TYPE2JOURNAL.get, inv_types)))
        if self._context.get('journal_type', False) == 'sale_add':
            tp = ('type', 'in', ['sale_add'])
        domain = [tp, ('company_id', '=', company_id)]
    ...
    def _get_journal_domain(self):
        inv = self or (self.browse(self._context.get('active_id', [])))
        try:
            inv_tp = (inv._context.get('default_type', False)) or inv.type
        except MissingError:
            inv_tp = 'out_invoice'
        tp = {'out_invoice': ['sale'], 'out_refund': ['sale_refund'], 'in_refund': ['purchase_refund'],
              'in_invoice': ['purchase']}.get(inv_tp, [])
        if inv._context.get('journal_type', False) == 'sale_add':
            tp = ['sale_add']
        company_id = inv.env.user.company_id.id
        return [('type', 'in', tp), ('company_id', '=', company_id)]
    ...
    journal_id = fields.Many2one('account.journal', string='Journal',
        required=True, readonly=True, states={'draft': [('readonly', False)]},
        default=_default_journal, domain=_get_journal_domain)
    ...
    def onchange_company_id(self, company_id, part_id, type, invoice_line, currency_id):
        ...
        if company_id and type:
            ...
            if not values.get('journal_id'):
                ...
                if type_label == 'sale_add':
                    type_label = u'Adición en Venta'
        ...
        if not self.tax_line:
            ...
            for tax in self.tax_line:
                if tax.manual:
                    continue
                try:
                    partner_id2 = tax.partner_id_2
                except:
                    partner_id2 = False
                if partner_id2:
                    key = (tax.tax_code_id.id, tax.base_code_id.id, tax.account_id.id, tax.account_analytic_id.id, tax.partner_id_2.id)
                else:
...
class account_invoice_line(models.Model):
    ...
    def product_id_change(self, product, uom_id, qty=0, name='', type='out_invoice',
            ...
            company_id=None, account_analytic_id=False):
        context = self._context
    ...
    def move_line_get_item(self, line):
        aa_id = line.account_analytic_id.id
        if 'config_analytic_global' in line.company_id._all_columns:
            self.env.cr.execute("SELECT aat.report_type, rc.config_analytic_global FROM account_account aa INNER JOIN "
                                "account_account_type aat ON aa.user_type = aat.id INNER JOIN res_company rc ON "
                                "aa.company_id = rc.id WHERE aa.id = %s" % line.account_id.id)
            analy_acc = self.env.cr.fetchone()
            aa_id = aa_id if analy_acc[0] in ['income', 'expense'] or analy_acc[1] else False
        return {
            'type': 'src',
