...
class stock_move(osv.osv):
    ...
    @api.cr_uid_ids_context  # TODO interesting
    ...
    def action_done(self, cr, uid, ids, context=None):
        ...
        for picking in picking_obj.browse(cr, uid, list(pickings), context=context):
            initial_values = {picking.id: {'state': 'assigned'}}
            if picking.state == 'done':
                ctx = context.copy()
                ctx['force_mail'] = True
                picking_obj.message_track(cr, uid, picking.id,
                                          picking_obj.fields_get(cr, uid, ['state'], context=context), initial_values,
                                          context=ctx)
                # Notificacion
                pick_obj = self.pool.get('stock.picking')
                author = self.pool.get('res.users').browse(cr, uid, uid, context).partner_id.id
                post_vars = {
                    'subject': "Transferencia terminada",
                    'body': "El picking ha cambiado de estado a transferido",
                    'partner_ids': [x.id for x in picking.message_follower_ids],
                    'subtype': "mt_comment",
                    'thread_id': picking.id,
                    'author_id': author,
                }
                pick_obj.message_post(cr, uid, **post_vars)
...
class stock_inventory_line(osv.osv):
    ...
    def _get_theoretical_qty(self, cr, uid, ids, name, args, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            res[line.id] = tot_qty
            if getattr(line, 'to_move_qty', None):
                prod_qty = tot_qty + line.to_move_qty
                self.write(cr, uid, line.id, {'product_qty': prod_qty}, context=context)
        return res
