        ...
        for product, qty, partner in products_by_qty_by_partner:
            ...
            for rule in items:
                ...
                if rule.base == -1:
                    ...
                    price_uom_id = qty_uom_id
                    if rule.price_version_id.tasa_cambio_pactada != 0 and rule.price_version_id.tasa_cambio_pactada != 1:
                        ctx = context.copy()
                        ctx.update({'pricelist_rate':rule.price_version_id.tasa_cambio_pactada})
                        context = ctx
