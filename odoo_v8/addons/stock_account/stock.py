...
class stock_picking(osv.osv):
    ...
    def _get_invoice_vals(self, cr, uid, key, inv_type, journal_id, move, context=None):
        ...
        if inv_type in ('out_invoice', 'out_refund'):
            ...
            acc_type = 'por pagar'
        if partner and not account_id:
            raise osv.except_osv('Error!', (u'Por favor configure la cuenta %s en el tercero %s' %
                                            (acc_type, partner.name)))
        return {
    ...
    def _invoice_create_line(self, cr, uid, moves, journal_id, inv_type='out_invoice', context=None):
        ...
        for move in moves:
            ...
            origin = move.picking_id.name
            partner, user_id, currency_id = move_obj._get_master_data(cr, 1, move, company, context=context)
